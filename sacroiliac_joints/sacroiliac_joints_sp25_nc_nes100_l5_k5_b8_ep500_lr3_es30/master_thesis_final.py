...
Parameters
"""
SUPERPIXELS_COUNT = 25
DATASET_DIR = 'sacroiliac_joints'
...
class InRAMMemoryDataset(InMemoryDataset):
  ...
  def process(self):
    ...
    grayscaled_images = torch.tensor(np.array(grayscale_images(ungrayscaled_images))) \
...
def image_to_pyg_Data(image):
  ...
  node_feature_vectors = [
    [pixel_intensity] for i, row in enumerate(image) for j, pixel_intensity in enumerate(row)
  ]
...
def grayscale_image(ungrayscaled_image):
  try:
    grayscaled_image = (
      ...
      print("An error occurred while grayscaling image: ", error)
...
dataset = InRAMMemoryDataset(
  root=f'/net/tscratch/people/plgdklarenbach/data/{DATASET_DIR}/{SUPERPIXELS_COUNT}_nc',
