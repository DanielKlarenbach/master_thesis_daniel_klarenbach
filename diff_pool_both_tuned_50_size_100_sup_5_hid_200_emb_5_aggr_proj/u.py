...
class Model(torch.nn.Module):
  ...
  def forward(self, x, edge_index, batch):
    ...
    class_probs, mask = to_dense_batch(class_probs, batch)
    adj = to_dense_adj(edge_index, batch)
    _, _, l1, l2 = dense_diff_pool(class_probs, adj, class_probs, mask)

    return torch.nn.Softmax(dim = -1)(class_probs), l1 * 100000 + l2, l2
...
def train():
  ...
  for batch_index, data in enumerate(train_dataloader):
    ...
    l1_e += (l1 - l2).item() * current_batch_size
...
def test(dataloader):
  ...
  for data in dataloader:
    ...
    batch_under_error, batch_superpixels_sizes_std, batch_inter_variance, batch_disjointed_percentage, batch_disjointed_mean_parts, batch_superpixels = \
      batch_compute_matrics(
        image_batch, superpixels_labels_batch, segments_labels_batch
      )
...
plot_with_different_scales(
  l1_a,
  l2_a,
  'Link prediction and entropy regularization loss',
...
for data in test_dataset:
    ...
    segments_labels = data.segments_labels
    number_of_sup = len(np.unique(superpixels_labels))
    slic_superpixels += number_of_sup

    under_error, _ = compute_undersegmentation_error_and_superpixels_sizes_std(
      superpixels_labels, segments_labels
    )

    underseg_error += under_error
    inter_variation += compute_intra_cluster_variation(image, superpixels_labels)

print(f'SLIC undersegmentation error: {underseg_error / len(test_dataset)}')
print(f'SLIC inter-cluster variation: {inter_variation / slic_superpixels}')
print(f'SLIC number of superpixels: {slic_superpixels}')
print(f'SLIC superpixels per image: {slic_superpixels/len(test_dataset)}')
...
for data in test_dataloader:
  data = data.to(device)
  preds, loss, _ = model(data.x, data.edge_index, data.batch)

end_inference_time = time.time()

total_time = end_inference_time - start_inference_time
print(f"Inference time: {total_time / len(test_dataloader.dataset)} seconds")

underseg_error = 0
inter_variation = 0
orginal_superpixel_count = 0
relabeled_superpixels_count = 0

def predictions_to_labels(batch_predictions):
  superpixel_labels = []
  assert len(batch_predictions) == 1
  graph_predictions = batch_predictions[0]

  for node_predictions in graph_predictions:
...
for data in test_dataset:
    ...
    relabeled_superpixels_labels = torch.tensor(label(predicted_superpixel_lables, connectivity=2))

    relabeled_superpixels_count += \
      len(torch.unique(torch.flatten(relabeled_superpixels_labels)))
    orginal_superpixel_count += \
      len(torch.unique(torch.flatten(torch.tensor(predicted_superpixel_lables))))
     
    image = data.image
    segments_labels = data.segments_labels

    under_error, _ = compute_undersegmentation_error_and_superpixels_sizes_std(
      relabeled_superpixels_labels, segments_labels
    )

    underseg_error += under_error
    inter_variation += compute_intra_cluster_variation(image, relabeled_superpixels_labels)

print(f'Relabeled undersegmentation error: {underseg_error / len(test_dataset)}')
print(f'Relabeled inter-cluster variation: {inter_variation / relabeled_superpixels_count}')  
print(f'Relabeled mean superpixels count: {relabeled_superpixels_count / len(test_dataset)}')  
print(f'Original mean superpixels count: {orginal_superpixel_count / len(test_dataset)}')  
...
display_results(test_dataset, 0, 20)
print('Done!')
